...
export default function LaunchpadPage() {
  ...
  const fetchTokens = useCallback(async (t: Tab) => {
    setLoading(true);
    try {
      ...
      const data = await apiFetch(`/api/pumpfun/coins?offset=0&limit=30&sort=${sort}&order=DESC${params}`);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => fetchTokens(tab), 5 * 60 * 1000);
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <p className="text-slate-400 mt-1">Real-time Solana meme token launches from PumpFun + DexScreener</p>
      </motion.div>
      ...
      {loading && tokens.length === 0 ? (
        ...
        <div className="glass-card p-8 text-center">
          <p className="text-slate-400">No recent tokens found. Try refreshing.</p>
        </div>
      ) : (
          ...
          {tokens.map((t: any) => (
            ...
            >
              <div className="flex items-start gap-3 mb-3">
                {t.image_uri && (
                  <img src={t.image_uri} alt={t.symbol} className="w-12 h-12 rounded-lg object-cover"
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {t.source === "dexscreener" && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded bg-purple-500/10 text-purple-400">DEX</span>
                    )}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs mb-3">
                ...
                {t.volume_24h > 0 && (
                  <div>
                    ...
                    <p className="text-white">${Number(t.volume_24h).toLocaleString(undefined, { maximumFractionDigits: 0 })}</p>
                  </div>
                )}
                {t.price_change_5m !== undefined && t.price_change_5m !== 0 && (
                  <div>
                    <p className="text-slate-500">5m Change</p>
                    <p className={t.price_change_5m >= 0 ? "text-emerald-400" : "text-red-400"}>
                      {t.price_change_5m >= 0 ? "+" : ""}{t.price_change_5m.toFixed(1)}%
                    </p>
                  </div>
                )}
                {t.price_change_24h !== undefined && t.price_change_24h !== 0 && (
                  <div>
                    <p className="text-slate-500">24h Change</p>
                    <p className={t.price_change_24h >= 0 ? "text-emerald-400" : "text-red-400"}>
              ...
              <div className="flex gap-2">
                <a href={`https://pump.fun/${t.mint}`} target="_blank" rel="noopener noreferrer"
